...
int main(int argc, char *argv[])
{
    ...
    auto onMessage = [&](const void *data)
    {
        ...
        lastMsg = msg->data();
        received++;
        YOMK_INFO_TAG("TestRpcService", "[RECV] 收到消息: ", msg->data());
    };
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        {
            allPublished = false;
            YOMK_ERROR_TAG("TestRpcService", "[SEND] 发布失败: ", msg.data(), " status=", static_cast<int>(resp.m_status));
            break;
        }
        YOMK_INFO_TAG("TestRpcService", "[SEND] 发布消息: ", msg.data());
    ...
    YOMK_INFO_TAG("TestRpcService", "[RESULT] 接收=", recvCount, "/5 最后一条=", lastMsgCopy);
